...
if ($batch_id != '' && $exam_id == 'null') {
    $sql1 = "SELECT * FROM module_lecturer_tbl WHERE lecturer_id = '" . $_SESSION['memberid'] . "'";
    $result1 = mysqli_query($con, $sql1);
    if (mysqli_num_rows($result1)) {
        while ($row1 = mysqli_fetch_array($result1)) {
            $sql2 = "SELECT * FROM exam_tbl WHERE module_id = '" . $row1['module_id'] . "' AND batch_id = '" . $batch_id . "'";
            $result2 = mysqli_query($con, $sql2);
            if (mysqli_num_rows($result2)) {
                while ($row2 = mysqli_fetch_array($result2)) {
                    $sql3 = "SELECT * FROM exam_result_tbl WHERE exam_id = '" . $row2['exam_id'] . "'";
...
?>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        if ($send_coordinator == '0') {
                                            echo "<form id='form1' name='form1' method='post' action='lecturer-edit_exam_results_send_coordinator.php?student_id=$row3[student_id]&exam_id=$row3[exam_id]&status=$send_coordinator'>"
                                        ?>
    ...
    } elseif ($batch_id != '' && $exam_id != 'null') {
        $sql1 = "SELECT * FROM module_lecturer_tbl WHERE lecturer_id = '" . $_SESSION['memberid'] . "'";
    $result1 = mysqli_query($con, $sql1);
    if (mysqli_num_rows($result1)) {
        while ($row1 = mysqli_fetch_array($result1)) {
            $sql2 = "SELECT * FROM exam_tbl WHERE module_id = '" . $row1['module_id'] . "' AND batch_id = '" . $batch_id . "' AND exam_id = '" . $exam_id . "'";
            $result2 = mysqli_query($con, $sql2);
            if (mysqli_num_rows($result2)) {
                while ($row2 = mysqli_fetch_array($result2)) {
                    $sql3 = "SELECT * FROM exam_result_tbl WHERE exam_id = '" . $row2['exam_id'] . "'";
