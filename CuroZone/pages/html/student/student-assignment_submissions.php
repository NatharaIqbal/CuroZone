...
?>
	...
	<body>
		...
		<div class="mainframe">
			...
			<?php
			$sql = "SELECT * FROM assignment_tbl WHERE batch_id='" . $_SESSION['batch_id'] . "'";
			$result = mysqli_query($con, $sql);
			if (mysqli_num_rows($result)) {
			?>
				<table width="100%" align="center" class="filltbl">
					<thead class="titles">
						<tr>
							<th width="9%">Assignment ID</th>
							<th>Batch ID</th>
							<th>Module ID</th>
							<th>Type</th>
							<th>Due Date</th>
					...
					<tbody>
						...
						while ($row = mysqli_fetch_array($result)) {
							$sql1 = "SELECT * FROM module_tbl WHERE module_id = '" . $row['module_id'] . "'";
						...
						?>
							<tr>
								...
								<td><?php echo $row['module_id'], ' ', $row1['name']; ?></td>
								<?php $type = $row['type'];
								if ($type == "0") {
								...
								<td>
									<?php echo "<form id='form2' name='form2' method='post' action='student-upload_assignment.php?assignment_id=$row[assignment_id]&batch_id=$row[batch_id]'>" ?>
									<button type="submit" class="assign_btn" name="assignbtn" id="assignbtn">Upload</button>
									</form>
								</td>
							</tr>
						<?php
