...
?>
    ...
    <head>
        <meta charset="utf-8">
        <title>Assign Lecturers for Modules - CuroZone</title>
    ...
    <body>
        ...
        <div class="mainframe">
            ...
            <div class="welcome">
                <h2 style="text-align: center; padding: 5px;">Assign Lecturers for Modules</h2>
                ...
                $sql2 = "SELECT * FROM course_tbl WHERE course_id = '" . $course_id . "'";
                $result2 = mysqli_query($con, $sql2);
                $row2 = mysqli_fetch_assoc($result2);
                $department_id = $row2['department_id'];
                ?>
                <form id="form1" name="form1" method="post" action="program_manager-assign_module_check.php">
                    <table width="40%" class="filltbl">
                        <tbody>
                            <tr>
                                <td style="width: 45%; text-align: right"><label>Course ID</label></td>
                                <td style="width: 5%; text-align: center">:</td>
                                <td style="width: 50%; text-align: left"><label><?php echo $course_id; ?></td>
                            </tr>
                            <tr>
                                <td style="text-align: right"><label>Module ID</label></td>
                                <td style="text-align: center">:</td>
                                <td style="text-align: left"><label><?php echo $module_id; ?></td>
                            </tr>
                            <tr>
                                <td style="text-align: right"><label for="select4">Lecturer</label></td>
                                <td style="text-align: center">:</td>
                                <td style="text-align: left"><select name="lecturerselect" id="lecturerselect">
                                        <option value="" selected disabled>--Select--</option>
                                        <?php
                                        $sql3 = "SELECT * FROM employee_tbl WHERE role='2' AND department_id='" . $department_id . "'";
                                        $result3 = mysqli_query($con, $sql3);

                                        while ($row3 = $result3->fetch_assoc()) {
                                            $employee_id = $row3['employee_id'];
                                            $employee_fname = $row3['first_name'];
                                            $employee_lname = $row3['last_name'];
                                        ?>
                                            <option value="<?php echo $row3['employee_id'] ?>"><?php echo "{$employee_fname}  {$employee_lname}"; ?></option>
                                        <?php
                                        }
                                        ?>
                                    </select></td>
                            ...
                            <tr>
                                <td class="submit-btn" colspan="3" style="text-align: center"><input type="submit" name="submit" id="submit" value="Assign"></td>
                            </tr>
                        </tbody>
                    </table>
                </form>
